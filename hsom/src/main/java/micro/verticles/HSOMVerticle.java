...
import micro.HSOMRESTAPI;
import micro.datastreamers.DataStreamerService;
import micro.entity.HSOM;
import micro.entity.HSOMNode;
import micro.service.HSOMCoreService;
import micro.service.HSOMMapService;
import micro.service.HSOMService;
...
public class HSOMVerticle extends AbstractVerticle{

    private static Logger log = LoggerFactory.getLogger(HSOMVerticle.class);
    private HSOMService service;
    ...
    */
    ...
    public void start() throws Exception {
        super.start();
        //initService();
        service = new HSOMCoreService(vertx, config(), log);
        ...
        // Routes. The actual API is defined in HSOMRESTAPI.java file.
        router.get(HSOMRESTAPI.GET_ALL).handler(this::getAllHSOM);
        router.post(HSOMRESTAPI.ADD_HSOM).handler(this::addHSOM);
        router.delete(HSOMRESTAPI.DELETE_HSOM).handler(this::deleteHSOM);
        router.get(HSOMRESTAPI.GET_HSOM).handler(this::getHSOM);
        router.get(HSOMRESTAPI.GET_NODE).handler(this::getNode);
        router.post(HSOMRESTAPI.ADD_NODE).handler(this::addNode);
        router.delete(HSOMRESTAPI.DELETE_NODE).handler(this::deleteNode);
        router.post(HSOMRESTAPI.ADD_EDGE).handler(this::connectNode);

        vertx.createHttpServer().requestHandler(router::accept).listen(port, ar -> {
            if(ar.succeeded())
                log.info("HSOM service deployed on http://localhost:" + port + ".");
            else
                log.info("Failed to deploy HSOM service...\n" + ar.cause());
        });
    }
    ...
    /**
     *  This method is responsible to retrieve all the existing HSOM models.
     *
     * @method GET
     * @path /hsoms
     * @param routingContext
    ...
    /**
     *  This method is responsible to add a new HSOM entity.
     *
     * @method POST
     * @path /hsoms
     * @param routingContext
    ...
    private void addHSOM(RoutingContext routingContext){
        ...
        final HSOM hsom = new HSOM(hsomName);
        service.addHSOM(hsom).setHandler(resultHandler(routingContext, res -> {
            if(res != null){
    ...
    /**
     *  This method is responsible to delete the HSOM entity with the id = :id
     *
     * @method DELETE
     * @path /hsoms/:id
     * @param routingContext
    ...
    /**
     * This method is responsible to return the HSOM entity with the id = :id
     *
     * @method GET
     * @path /hsoms/:id
     * @param routingContext
    ...
    /**
     *  This method is responsible to return the node with the node_id = :nodeid from the HSOM with the id = :id
     *
     * @method GET
    ...
    /**
     *  This method is responsible to add a new node to the HSOM with the id = :id
     *
     * @method POST
     * @path /hsoms/:id/nodes
     * @param routingContext
    ...
    private void addNode(RoutingContext routingContext){
        ...
        HSOMNode node = new HSOMNode();
        //TODO: Check he JSON to HSOMNode conversion
        service.addNode(hsomID, node).setHandler(resultHandler(routingContext, res -> {
            if(res != null){
                final String encoded = res.toJson().put("model", res.toJson()).encodePrettily();
                routingContext.response()
                        .setStatusCode(201)
                        .putHeader("content-type", "application/json")
                        .end(encoded);
    ...
    /**
     *  This method is responsible to the node with the node_id = :nodeid off the HSOM with the id = :id
     *
     * @method DELETE
     * @path /hsoms/:id/nodes/:nodeid
     * @param routingContext
    ...
    /**
     *  This method is responsible to add an edge to the HSOM with the id = :id.
